# Copyright 2025, ML-AI contributors
# Authors: Abdou Diaw (ORNL)
# License: 3-Clause-BSD-LBNL
"""
This file is part ML-AI project.
...
def getAllGNDData(dbPath):
    """Fetch all SOLPS data from the SQLite database."""

    selString = ""

    selString = "SELECT * FROM SOLPSDIIID;"
...
class Model():
    ...
    def iserrok(self, errbars):
        if isinstance(errbars, Outputs):
            packed = self.pack_outputs(errbars)
        else:
            packed = errbars  # assume it's already packed

        result = self.process_iserrok(packed)

        # Only unpack if result is a flat array
        if isinstance(result, np.ndarray):
            return self.unpack_outputs(result)
        return result  # assume already unpacked or boolean
    ...
    def iserrok_fuzzy(self, errbars):
        ...
        result = self.process_iserrok_fuzzy(packed)
...
class LearnerModel(Model):
    def pack_inputs(self,request):
        packed_request = np.concatenate([[request.gas_puff], [request.p_tot], [request.core_flux],
                                         [request.dna], [request.hci], [request.r], [request.region]])
    ...
    def pack_outputs(self,outputs):
        output_vals = np.concatenate([[outputs.ne], [outputs.te],
                                      [outputs.ti], [outputs.po]])
        return output_vals


# Parameters governing input and outputs to problem
SOLVER_INDEXES = dict(
        input_slice = slice(0,7),
        output_slice = slice(7,11),
        n_inputs = 7,
        n_outputs = 4,
)
...
def train_single_model(train_data, learning_config):
    ...
    scheduler = training_config["scheduler_type"](opt,patience=patience,factor=0.5)
